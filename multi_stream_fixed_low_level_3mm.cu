...
int main(int argc, char* argv[]) {
	...
	if (pthread_create(&tid[0], NULL, free_in_thread, (void*)&VMM_A) < 0) {
		perror("pthread 0 create error\n");
		exit(EXIT_FAILURE);
	}
	if (pthread_create(&tid[1], NULL, free_in_thread, (void*)&VMM_B) < 0) {
		perror("pthread 0 create error\n");
		exit(EXIT_FAILURE);
	}
	mm3_kernel2<<<grid2,block, 0, stream[1]>>>(C_gpu, D_gpu, F_gpu);
	// low_level_free(VMM_A);
	// low_level_free(VMM_B);0
	...
	if (pthread_create(&tid[2], NULL, free_in_thread, (void*)&VMM_C) < 0) {
		perror("pthread 0 create error\n");
		exit(EXIT_FAILURE);
	}
	if (pthread_create(&tid[3], NULL, free_in_thread, (void*)&VMM_D) < 0) {
		perror("pthread 0 create error\n");
		exit(EXIT_FAILURE);
	}
