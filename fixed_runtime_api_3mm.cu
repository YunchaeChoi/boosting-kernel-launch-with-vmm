...
int main(int argc, char* argv[]) {
	...
	*/
	...
	cudaFree(A_gpu);
	cudaFree(B_gpu);
	// pthread_create(&tid[0], NULL, free_in_thread, &A_gpu);
	// pthread_create(&tid[1], NULL, free_in_thread, &B_gpu);
	mm3_kernel2<<<grid2,block>>>(C_gpu, D_gpu, F_gpu);
	cudaDeviceSynchronize();
	cudaFree(C_gpu);
	cudaFree(D_gpu);
	// pthread_create(&tid[2], NULL, free_in_thread, &C_gpu);
	// pthread_create(&tid[3], NULL, free_in_thread, &D_gpu);
